...
- **固定列**：`fixed: 'left' | 'right'` 冻结列，横向滚动时保持可见，自动阴影分隔
- **排序**：`sortable: true` 点击表头切换升序/降序，内置排序指示箭头
- **行选择**：支持单选/多选模式，点击行自动高亮，提供 `selectAll`、`clearSelection` 等方法
- **scrollTo API**：`scrollToRow(index, align)` 精确定位到指定行，支持 start/center/end 对齐
